...
### Drop covariates to see if PS overlap improves ----
  ...
  # transforming dollar3 into quartiles
  subset <- subset(restaurant, (treat==1&policy=="suffolk")|treat==0)
  subset$entry <- 251
  tmp <- subset %>% group_by(address) %>% mutate(relative = monthno - entry) %>%
    filter(relative<0&relative>=-24) %>% mutate(n=n()) %>% mutate(open24 = ifelse(n==24, 1,0)) %>%
  ...
  quantile(subset$dollar3[subset$treat=="1"])
  #min =6.674042
  #q1 = 7.298081
  #q2 = 7.665343
  #q3 =8.068349
  ...
  quantile(subset$dollar3[subset$treat=="0"])
  
 
  subset <- subset %>%
            mutate(dollar3 = ifelse((dollar3 >= 6.674042) & (dollar3 < 7.298081), "a", dollar3)) %>%
            mutate(dollar3 = ifelse((dollar3 >= 7.298081) & (dollar3 < 7.665343), "b", dollar3)) %>%
            mutate(dollar3 = ifelse((dollar3 >= 7.665343) & (dollar3 < 8.068349), "c", dollar3)) %>%
            mutate(dollar3 = ifelse((dollar3 >= 8.068349) & (dollar3 <= 9.559823), "d", dollar3)) %>%
            filter((dollar3 == c("a", "b", "c", "d"))) %>%
            mutate(dollar3 = ifelse(dollar3 == "a", 1, dollar3)) %>%
            mutate(dollar3 = ifelse(dollar3 == "b", 2, dollar3)) %>%
            mutate(dollar3 = ifelse(dollar3 == "c", 3, dollar3)) %>%
            mutate(dollar3 = ifelse(dollar3 == "d", 4, dollar3)) 
  ...
  tryCatch({#catch groups that do not have comparison restaurants
    ...
    #subset <- subset %>%
    #  mutate(dollar3 = ifelse((dollar3 >= min(subset$dollar3[subset$treat=="1"])) & (dollar3 < median(subset$dollar3[subset$treat=="1"])), 0, dollar3)) %>%
    #  mutate(dollar3 = ifelse((dollar3 >= median(subset$dollar3[subset$treat=="1"])) & (dollar3 <= max(subset$dollar3[subset$treat=="1"])), 1, dollar3)) %>%
    #  filter(dollar3 %in% c(0,1))
    
    #subset <- subset %>%
    #  mutate(dollar3 = ifelse((dollar3 < median(subset$dollar3[subset$treat=="1"])), 0, dollar3)) %>%
    #  mutate(dollar3 = ifelse((dollar3 >= median(subset$dollar3[subset$treat=="1"])), 1, dollar3))
    
    #subset$dollar3 <- as.numeric(subset$dollar3)
    
    #subset <- subset %>%
    #  mutate(calorie3 = ifelse((calorie3 >= min(subset$calorie3[subset$treat=="1"])) & (calorie3 < median(subset$calorie3[subset$treat=="1"])), 0, calorie3)) %>%
    #  mutate(calorie3 = ifelse((calorie3 >= median(subset$calorie3[subset$treat=="1"])) & (calorie3 <= max(subset$calorie3[subset$treat=="1"])), 1, calorie3)) %>%
    #  filter(calorie3 %in% c(0,1))
    
    #subset <- subset %>%
    #  mutate(calorie3 = ifelse((calorie3 < median(subset$calorie3[subset$treat=="1"])), 0, calorie3)) %>%
    #  mutate(calorie3 = ifelse((calorie3 >= median(subset$calorie3[subset$treat=="1"])), 1, calorie3))    
    #         
    #
    #subset$calorie3 <- as.numeric(subset$calorie3)
...
### Analysis ----
  ...
  #overall
  mod.factor <- plm(formula = calorie~treat*relative2_factor+as.factor(month),

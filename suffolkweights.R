...
### Alternative cleaning - simple imputation of mean ----
  ...
  #              group_by(address) %>%
  #              mutate(meancal = mean(calorie)) %>%
  #              mutate(calorie = ifelse(calorie < meancal*0.5, meancal, calorie)) %>%
  #              dplyr::select(-meancal)
  #
  ## Checking
  #todrop <- restaurant2 %>%
  #          mutate(relative2 = monthno - entry) %>% # defines relative month
...
### Matching procedure 1 - significant predictors of outcome ----
  ...
  table3 <- table3 %>%
    ...
    add_column(col="",.before=4) %>%
    mutate_all(~replace(., is.na(.), ""))
  write.csv(table3, "manuscript/tables/table3-match1-sigpred-vif.csv")
  rm(treat,comp,mod.factor,table3,p,presum,tidy_mod.factor)
...
### Matching procedure 2 - match on only calorie and dollar vars ----
  ...
  for (i in c(1:10)) {
    ...
    print(vif_diag)
      ...
      ### vif diagnostic ----
  formula <- treat ~ calorie3  + dollar3  #dropping _all works
  ...
  table3 <- table3 %>%
    ...
    add_column(col="",.before=4) %>%
    mutate_all(~replace(., is.na(.), ""))
  write.csv(table3, "manuscript/tables/table3-match2-sigpred-vif.csv")
  rm(treat,comp,mod.factor,table3,p,presum,tidy_mod.factor)
...
### Matching procedure 3 - match on all 8 calorie and dollar vars ----
  ...
  for (i in c(1:10)) {
    ...
    print(vif_diag)
      ...
      ### vif diagnostic ----
  formula <- treat ~ calorie3 + dollar3  +
    slope_calorie + slope_dollar   #dropping _all works
  ...
  table3 <- table3 %>%
    ...
    add_column(col="",.before=4) %>%
    mutate_all(~replace(., is.na(.), ""))
  write.csv(table3, "manuscript/tables/table3-match3-sigpred-vif.csv")
  rm(treat,comp,mod.factor,table3,p,presum,tidy_mod.factor)
...
### Matching procedure 4 - match on all 8 calorie and dollar vars, plus restaurant-level characteristics ----
  ...
  table3 <- table3 %>%
    ...
    add_column(col="",.before=4) %>%
    mutate_all(~replace(., is.na(.), ""))
  write.csv(table3, "manuscript/tables/table3-match4-sigpred-vif.csv")
  rm(treat,comp,mod.factor,table3,p,presum,tidy_mod.factor)
...
### Matching procedure 5 - significant predictors of outcome for full model ----
  ...
  # Significant predictors, stratified by location
    ...
    for (i in c(1:10)) {
      ...
      subset_all <- rbind(subset_all, subset) %>% distinct()
    }
  ...
  # New formula (predictors of overall model)
    ...
    write.csv(table3, "manuscript/tables/table3-match5-overallsigpred.csv")
    rm(treat,comp,mod.factor,table3,p,presum,tidy_mod.factor)
    ...
    formula <- treat ~ concept + ownership + drive + calorie3 + slope_calorie + count3 +
      slope_count + dollar3 + slope_dollar + total + asian + median_income + open12 + open18
        ...
        write.csv(table3, "manuscript/tables/table3-match5-overallsigpred-vif.csv")
        rm(treat,comp,mod.factor,table3,p,presum,tidy_mod.factor)
...
### Estimates from unmatched model ----
    ...
    write.csv(table3, "manuscript/tables/table3-unmatched.csv")
    rm(treat,comp,mod.factor,table3,p,presum,tidy_mod.factor)

...
ggplot(data=trend,aes(x=month, y=calorie,color=as.character(group))) + 
  ...
  geom_point(aes(x=14.8, y=-200), color='orange', size=1) +
  coord_cartesian(expand = FALSE, clip = "off") + 
  scale_y_continuous(limits=c(-400,200),breaks=seq(-400,200,50),
                     sec.axis = sec_axis(~(.+200)/1, name="Difference-in-difference estimate")) +
...
# Equality of DiD estimates, 3-12 month, by location ----------------------
for (p in c("ca","ma","or","suffolk")) {
  mod.factor[[p]] <- plm(formula = calorie~treat*relative2.factor+as.factor(month),
                         data = matched%>%filter(((relative2>=-30&relative2<=-3)|(relative2>=2&relative2<=29))&match_place==p), 
                         index = "id_match", weights = weights, model = "within")
  
  tidy_mod.factor[[p]] <- tidy(mod.factor[[p]],conf.level = 0.95,conf.int = TRUE) %>%
    dplyr::select(term,estimate,p.value,conf.low,conf.high) %>%
    rename(month=term,coef.month=estimate,p=p.value,low=conf.low,high=conf.high) %>%
    filter(!grepl("as.factor|calorie", month)) %>%
    mutate(group=c(rep(0,55),rep(1,55))) %>%
  ...
  treat[[p]] <- tidy_mod.factor[[p]] %>% filter(group==1&month>=-8&month<=12) %>%
    mutate(post=ifelse(month<0,0,1)) %>% group_by(post) %>%
    summarise(calorie=mean(calorie),low=mean(low),high=mean(high)) %>% ungroup() %>%
    mutate(calorie=ifelse(post==1,calorie-calorie[1],calorie),low=ifelse(post==1,low-low[1],low),high=ifelse(post==1,high-high[1],high)) %>%
    filter(post==1)
  
  comp[[p]] <- tidy_mod.factor[[p]] %>% filter(group==0&month>=-8&month<=12) %>%
    mutate(post=ifelse(month<0,0,1)) %>% group_by(post) %>%
    summarise(calorie=mean(calorie),low=mean(low),high=mean(high)) %>% ungroup() %>%
    mutate(calorie=ifelse(post==1,calorie-calorie[1],calorie),low=ifelse(post==1,low-low[1],low),high=ifelse(post==1,high-high[1],high)) %>%
    filter(post==1)
...
# Creating matrix for mean, sd, n
equality <- data.frame(matrix(0, ncol = 4, nrow = 4))
colnames(equality) <- c("location","mean", "sd", "n")
equality$location <- c("ca", "ma", "or", "suffolk")
equality$n <- 8

# Constructing sd from CIs
equality[1,3] <- (sqrt(8) * (diff$ca$ca.high - diff$ca$ca.low) / 3.92)
equality[2,3] <- (sqrt(8) * (diff$ma$ma.high - diff$ma$ma.low) / 3.92)
equality[3,3] <- (sqrt(8) * (diff$or$or.high - diff$or$or.low) / 3.92)
equality[4,3] <- (sqrt(8) * (diff$suffolk$suffolk.high -  diff$suffolk$suffolk.low) / 3.92)

# Inserting means$
equality[1,2] <- diff$ca$ca.calorie
equality[2,2] <- diff$ma$ma.calorie
equality[3,2] <- diff$or$or.calorie
equality[4,2] <- diff$suffolk$suffolk.calorie

# Defining t-test function with sample statistics  
T.test <- function(mean, sd, n) {
...
combos <- combn(rownames(equality), 2, simplify=F)

# Running written t-test over all combinations  
out <- data.frame(sapply(combos, function(x){
  row1 <- equality[x[1], ]
  row2 <- equality[x[2], ]
  test_mat <- rbind(row1, row2)
  out <- data.frame(T.test(test_mat$mean, test_mat$sd, test_mat$n))
  
  out_tab <- data.frame("location1" = test_mat[x[1],1],
                        "location2" = test_mat[x[2],1],
                        "p.value" = sprintf("%.3f", out[2,1])
  )
  return(out_tab)
}))

colnames(out) <- c("pair1","pair2","pair3","pair4","pair5","pair6")

out <- apply(out, 2, as.character)
#write.csv(out, "manuscript/tables/table3_equality.csv")
